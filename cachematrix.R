...
# x$getInv # to GET the inverse matrix
										...
										## The function cacheSolve fetches the Matrix Inverse if its available, otherwise it 
## computes the Matrix Inverse and returns the same

										cacheSolve <- function(x, ...) {
   ## Return a matrix that is the inverse of 'x'
    m_inv <- x$getInv() # To get the INVERSE MATRIX from object x
										     if(!is.null(m_inv)) { # To check if the INVERSE MATRIX is already available
  ...
  # To generate or fetch calculated inverse matrix using the cacheSolve()
